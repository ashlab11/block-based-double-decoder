...
def main():
    ...
    parser.add_argument("--tokenizer-file", default="tokenizer/tokenizer.json")
    ...
    print(f"▶ Retrieving {args.tokens // 1_000_000}M tokens from Ultrachat")
    ...
    with output_file.open("w", encoding="utf-8") as f, test_file.open("w", encoding="utf-8") as tgt:
        ...
        def process_buffer():
            ...
            for ids_inp, ids_out, fh in zip(ins, outs, buf_files):
                if (len(ids_inp) + len(ids_out) + 2) <= SEQ_LEN and tok_so_far < args.tokens:
    ...
    if tok_so_far >= args.tokens:
        ...
        print(f"\nWarning: Only {tok_so_far:,} tokens written (target was {args.tokens:,})")
